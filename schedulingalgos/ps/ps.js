...
function add_process(e) {
  ...
  let priority = parseInt(document.getElementById("priority").value, 10);
...
function run_algorithm(e) {
  ...
  let lp = parseInt(document.getElementById("lp").value, 10);
  if (rowLength === 1) {
    document.querySelector(".error").style.display = "block";
    document.querySelector(".error").innerHTML =
  ...
  if (!Number.isInteger(hp) || !Number.isInteger(lp)) {
    document.querySelector(".error").innerHTML =
      "Please enter Highest and Lowest priority";
    document.querySelector(".error").style.display = "block";
    setTimeout(function () {
      document.querySelector(".error").style.display = "none";
      document.querySelector(".error").innerHTML = "Invalid Input";
  ...
  for (let i = 1; i < rowLength; i++) {
    processArr.push({
      ...
      p:
        hp > lp
          ? hp - parseInt(table.rows.item(i).cells.item(3).innerHTML, 10)
          : parseInt(table.rows.item(i).cells.item(3).innerHTML, 10),
  ...
  for (let i = 0; i < processArr.length; i++) {
    ...
    for (let j = 0; j < 5; j++) {
      document.querySelector(
        `.table>table>tbody>tr:nth-child(${i + 1})>td:nth-child(${j + 5})`
...
function calculateAllTimes(arr) {
  ...
  while (arr.find((el) => el.finish == undefined)) {
    ...
    let process = {};
    for (let i = 0; i < arr.length; i++) {
      if (arr[i].at <= time && arr[i].finish != true && arr[i].p < highP) {
        highP = arr[i].p;
        process = arr[i];
      }
    }
